...
function Home() {
    ...
    const fetchData = async () => {
      try {
        const response = await fetch(
          "https://backendapi.trakky.in/salonvendor/dashboard/sales/",
          {
            method: "GET",
            headers: {
  ...
  useEffect(() => {
    const Staff = async () => {
      const response = await fetch(
        "https://backendapi.trakky.in/salonvendor/staff-monthly-detail/",
        {
          method: "GET",
          headers: {
            authorization: `Bearer ${token}`,
  ...
  return (
    <>
      <div className="POSgraphs flex flex-col items-center">
        <span className="w-full flex justify-center relative top-[25px]">
        ...
        <div>
          {activeTeam === "Sales" ? (
            <DashSales data={dashData} />
          ) : activeTeam === "Staff" ? (
            <DashStaff data={staffData} />
          ) : activeTeam === "Client" ? (
            <DashClient />
          ) : null}
        </div>
