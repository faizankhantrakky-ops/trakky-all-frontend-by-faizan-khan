...
const EditAppointment = ({
  ...
  closeDrawer,
  setAppointmentData,
  isEditMode = false, // New prop to distinguish between edit and checkout mode
}) => {
  ...
  const [
    finalTotalAppointmentAmountAfterTax,
    setFinalTotalAppointmentAmountAfterTax,
  ] = useState(0);
  const [finalTotalProductAmountAfterTax, setFinalTotalProductAmountAfterTax] =
    useState(0);
  ...
  useEffect(() => {
    if (isEditMode) {
      // In edit mode, start with Customer Details and make fields editable
      setActiveStep("Customer Details");
    } else {
      // In checkout mode, maintain original behavior
      setActiveStep("Customer Details");
    }
  }, [isEditMode]);
    ...
    const fetchStaff = async () => {
      try {
        ...
        if (response.ok) {
          ...
          // ✅ Only keep staff where: is_permanent = true AND is_present = true
          const presentStaff = Array.isArray(data)
            ? data.filter(
                (staff) =>
  ...
  const sendWhatsAppInvoice = async () => {
    ...
    try {
      // Enhanced debug: Log all key variables first
      console.log("Debug: appointment:", appointment);
      console.log("Debug: authTokens:", authTokens);
      console.log("Debug: vendorData:", vendorData);

      // Strict validation with better error messages
      if (!appointment) {
        throw new Error("appointment object is undefined or null");
      }
      if (!appointment.id) {
        throw new Error("appointment_id is missing from appointment");
      }
      if (!appointment.customer_phone) {
        throw new Error("customer_phone is missing from appointment");
      }
      ...
      if (!invoiceResponse.ok) {
        ...
        console.error("Invoice generation error:", errText);
        throw new Error(`Failed to generate invoice: ${errText}`);
      }
      ...
      if (!invoiceData.invoice_url) {
        throw new Error("invoice_url missing from response");
      }

      // Step 2: Clean and validate phone
      let customerPhone = (appointment.customer_phone || "").replace(
        /[^0-9]/g,
        "",
      );
      ...
      if (!customerPhone || customerPhone.length !== 10) {
        throw new Error(
          `Invalid customer_phone: ${appointment.customer_phone} (cleaned: ${customerPhone})`,
        );
      }

      // Step 3: Build payload with fallbacks (added appointment_id)
      const payload = {
        phone_numbers: [`91${customerPhone}`],
        appointment_id: appointment.id.toString(), // Ensure string if needed - THIS WAS MISSING!
        filename: `invoice_${appointment.id}.pdf`,
        file_url: invoiceData.invoice_url,
        body_values: [
          appointment.customer_name?.trim() || "Valued Customer",
          "Appointment",
          vendorData?.salon_name?.trim() || "Your Salon",
          vendorData?.salon_name?.trim() || "Your Salon",
        ],
      };
      ...
      console.log("Debug: WhatsApp Response Status:", whatsappResponse.status);
      console.log("Debug: WhatsApp Response Text:", responseText);
      ...
      console.error("Full Error in sendWhatsAppInvoice:", error);
      toast.error(
        error.message ||
          "Error sending invoice via WhatsApp. Check console for details.",
      );
    } finally {
      setIsGeneratingInvoice(false);
    }
  ...
  useEffect(() => {
    ...
    setFinalTotalAppointmentAmountAfterTaxDiscount(
      apptAfterDiscount + apptTaxAmt,
    );
    setFinalTotalProductSellAmountAfterDiscountTax(
      prodAfterDiscount + prodTaxAmt,
    );
    ...
    const combinedDiscPercentage =
      actualAmt > 0 ? (totalDiscountAmt / actualAmt) * 100 : 0;
    const combinedTaxPercentage =
      actualAmt > 0 ? (totalTaxAmt / actualAmt) * 100 : 0;
  ...
  const handleSubmit = async (e) => {
    ...
    // For edit mode, we don't want to process checkout or wallet operations
    if (isEditMode) {
      // Handle edit mode submission (PATCH request without checkout)
      ...
      // Build payload for edit update (without checkout)
      let payload = {
      ...
      try {
        setIsSubmitting(true); // ← Loader start (edit mode ke andar bhi)
        ...
        if (response.ok) {
          toast.success("Appointment Updated Successfully", "success");
          setAppointmentData(data);
          closeDrawer(); // Close the drawer after successful update
        } else {
        ...
        setIsSubmitting(false); // ← Loader band (hamesha)
      }
      return; // Exit early for edit mode
    }

    // Original checkout logic (for non-edit mode)
    let walletAmount = 0;
    ...
    try {
      setIsSubmitting(true); // ← Loader start (checkout ke liye)
      ...
      setIsSubmitting(false); // ← Loader band (dono mode mein)
    }
  };
    ...
    <>
      <div>
        <h2 className="text-xl font-semibold text-center md:mt-5">
          {isEditMode ? "Edit Appointment" : "Checkout Appointment"}
        </h2>
        ...
        <div className=" lg:max-w-[1000px] flex gap-2 h-full">
              ...
              {steps.map((step, index) => (
                <div
                  ...
                  className={`flex w-48 items-center gap-2 px-3 py-1 ${
                    ...
                    (index > 0 && (!customerPhone || !customerName))
                  ...
                  onClick={() => {
                    ...
                    if (index > 0 && (!customerPhone || !customerName)) {
                      return;
                    }
                    setActiveStep(step);
                  }}
          ...
          <div className=" mb-6  mt-5 h-fit overflow-y-auto  rounded-lg bg-white grid gap-x-6 gap-y-6 grid-cols-2 mx-auto py-5 md:py-4 lg:px-10 lg:w-[800px]">
              ...
              <>
                ...
                <div className=" w-full   ">
                  <TextField
                    ...
                    InputProps={{
                      readOnly: !isEditMode, // Make editable in edit mode
                    }}
                  />
                </div>
            ...
            {/* Rest of the component remains the same - Service Details, Product Consumption, etc. */}
            {/* ... (All the other steps and components remain unchanged) ... */}
              ...
              <>
                ...
                <div className="grid grid-cols-4 gap-3 w-full col-span-2">
                  ...
                  <TextField
                    ...
                    value={
                      Math.round(
                        finalTotalAppointmentAmountAfterDiscount * 100,
                      ) / 100
                    }
                    InputProps={{ readOnly: true }}
                    sx={{
                  ...
                  {(productFormData.final_total > 0 ||
                    ...
                    <TextField
                      ...
                      value={
                        Math.round(
                          finalTotalProductSellAmountAfterDiscount * 100,
                        ) / 100
                      }
                      InputProps={{ readOnly: true }}
                      sx={{
                ...
                <div className="grid grid-cols-4 gap-3 w-full col-span-2">
                    ...
                    <Select
                      ...
                      onChange={(e) => {
                        ...
                        if (newStatus === "unpaid") {
                          ...
                          // For partial, keep current amounts but ensure due is positive
                  ...
                  <FormControl fullWidth variant="standard">
                    ...
                    <Select
                      ...
                      onChange={(e) => {
                        ...
                        if (
                          ...
                          if (
                            !updatedDetails.find((d) => d.mode === "wallet")
                          ) {
                    ...
                    >
                      {/* Dynamic options from API - central_payment_method */}
                      {paymentMethods
                  ...
                  {!splitPaymentMode && (
                    <TextField
                      label="Amount Paid"
                      placeholder="Enter amount" // 👈 visible placeholder
                      type="number"
                      variant="standard"
                      fullWidth
                      value={amountPaid}
                      InputLabelProps={{ shrink: true }} // 👈 keeps label above the digits
                      onChange={(e) => {
                      ...
                      >
                        ...
                        {paymentMethods
                          ...
                          .map((method, index) => (
                            ...
                            >
                              <Checkbox
                                checked={selectedPaymentModes.includes(
                                  method.type,
                                )}
                              />
                              <ListItemText
                        ...
                        <div className="mt-2 p-2 bg-gray-50 rounded">
                          ...
                          {splitPaymentDetails.reduce(
                            ...
                            <Alert severity="warning" className="mt-1">
                              Amount not fully allocated. Remaining amount will
                              be added to due.
                            </Alert>
                          )}
                          ...
                          {splitPaymentDetails.reduce(
                            ...
                            <Alert severity="error" className="mt-1">
                              Total paid amount exceeds final amount! Please
                              adjust the amounts.
                            </Alert>
                          )}
                        </div>
                          ...
                          onClick={() => {
                            ...
                            const equalAmount =
                              nonWalletDetails.length > 0
                                ? remainingForNonWallet /
                                  nonWalletDetails.length
                                : 0;
                              ...
                              (detail) => {
                                if (detail.mode === "wallet") {
                                  return {
                                    ...detail,
                                    amount:
                                      Math.round(walletMaxAmount * 100) / 100,
                                  };
                                } else {
                                  return {
                                    ...detail,
            ...
            <div className="col-span-2 flex w-full justify-between items-center">
              ...
              {/* Save/Checkout button - shows different text based on mode */}
              <button
                className={`
    block py-2.5 px-6 w-fit font-medium rounded-md transition-all
    flex items-center justify-center gap-2 min-w-[140px]
    ${activeStep === "Checkout" ? "visible" : "invisible"}
    ${
      !paymentStatus && !isEditMode
        ? "bg-gray-400 cursor-not-allowed text-white opacity-70"
        : isSubmitting
        ? "bg-gray-600 cursor-not-allowed text-white opacity-80"
  ...
  `}
                onClick={(e) => {
                  ...
                  if (!paymentStatus && !isEditMode) return;
                  ...
                  if (isEditMode) {
                    ...
                    if (appointment?.checkout) {
                      // Already checked out → open invoice
                      setInvoicePage(true);
                    } else {
                ...
                disabled={(!paymentStatus && !isEditMode) || isSubmitting}
              >
                {isSubmitting ? (
                  <>
                    <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />
                    {isEditMode ? "Saving..." : "Checking out..."}
                  </>
                ) : isEditMode ? (
                  "Save Changes"
