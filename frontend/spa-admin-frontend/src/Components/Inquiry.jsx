...
const Inquiry = () => {
  ...
  const getInquiry = async () => {
    let url;
    if (searchTerm === "") {
      url = `https://backendapi.trakky.in/spas/log-entry/?page=${page}`;
    } else {
      url = `https://backendapi.trakky.in/spas/log-entry/?page=${page}&query=${encodeURIComponent(
        searchTerm.trim()
      )}`;
    }
    try {
                  ...
                  inquiryData.map((inquiry, index) => (
                    <>
                      <tr key={index}>
                        ...
                        <td>{inquiry.location.latitude || "-"}</td>
