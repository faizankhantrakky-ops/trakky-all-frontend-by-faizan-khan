...
const AddOffer = ({ offerData, setOfferData }) => {
  ...
  useEffect(() => {
    if (offerData) {
      const salonDatas = Object.keys(offerData?.salon_names || {}).map(
        (key) => {
          return { value: key, label: offerData?.salon_names[key] };
        }
      );
    ...
    if (offerData) {
      try {
        ...
        if (response.ok) {
          ...
          let errorMessage =
            errorData.slug !== undefined ? errorData.slug + " " : "";
          errorMessage +=
            errorData.city !== undefined ? errorData.city + " " : "";
          errorMessage +=
            errorData.area !== undefined ? errorData.area + " " : "";
          errorMessage +=
            errorData.img_url !== undefined ? errorData.img_url + " " : "";
          errorMessage +=
            errorData.salon !== undefined ? errorData.salon + " " : "";
          ...
          toast.error(
            `Error : ${response.status} - Server Side Error, try again later.`,
            {
              duration: 4000,
              position: "top-center",
              style: {
        ...
        <form method="post" onSubmit={handleSubmit}>
          ...
          <div className="row">
            <div className="input-box inp-id col-1 col-2">
              <label htmlFor="id">Select Offer Type</label>
              <select
                name="id"
                id="id"
                value={offerType}
              ...
              >
                {[
                  { label: "City Offer", value: "city" },
                  { label: "Area Offer", value: "area" },
                ].map((item, index) => (
            ...
            <div className="input-box inp-main-img col-1 col-2">
              <label htmlFor="img">
                ...
                <span className="Note_Inp_Classs">
                  Recommended Image Ratio 500px:200px
                </span>
              </label>
              <input
                type="file"
