...
const EnhancedCalendar = () => {
  ...
  const renderDays = () => {
    ...
    return <div className="grid grid-cols-7 gap-1 mb-1 min-w-[250px]">{days}</div>;
  };
      ...
      <div className="grid grid-cols-8 gap-1">
        ...
        {Array.from({ length: 24 }).map((_, hour) => (
          <React.Fragment key={hour}>
            <div className="bg-gray-100 p-2 text-sm">{`${hour.toString().padStart(2, "0")}:00`}</div>
            {days.map((day) => {
              const dateStr = formatDate(day);
              const dayAppointments = appointments.filter(
                (appointment) => {
                  const appointmentDate = parseISO(appointment.created_at);
                  return formatDate(appointmentDate) === dateStr && appointmentDate.getHours() === hour;
                }
              );
        ...
        {Array.from({ length: 24 }).map((_, hour) => (
          <React.Fragment key={hour}>
            ...
            >
              {dayAppointments
                .filter(
                  (appointment) => parseISO(appointment.created_at).getHours() === hour
                )
