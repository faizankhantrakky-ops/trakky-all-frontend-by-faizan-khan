...
function Appointment() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`https://backendapi.trakky.in/spavendor/appointments/filter-by-month/?month=${monthNum}&year=2024&vendor=6`, {
          method: 'GET',
          headers: {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`https://backendapi.trakky.in/spavendor/appointments/cancelled/filter-by-month/?month=${monthNum}&year=2024&vendor=6`, {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`
  ...
  return (
    <>
      <div className="daterangPickerPOS">
        <div onClick={handleOpenModal} className="TitleDaterangepickerPOSappointment">
          <Typography
            variant="body1"
            style={{
          ...
          >
            {`${moment(selectionRange.startDate).format('YYYY-MM-DD')} - ${moment(selectionRange.endDate).format('YYYY-MM-DD')}`}
          </Typography>
        </div>
        <Modal open={openModal} onClose={handleCloseModal} className="modalPOSappointmentDaterangepicker">
          <Box
            sx={{
              position: "absolute",
              top: "50%",
