...
const AppointmentListCard = ({ appointment }) => {
  ...
  const handleStartTimer = async () => {
    ...
    try {
      const res = await fetch(
        `https://backendapi.trakky.in/spavendor/appointments/${appointmentData?.id}/`,
        {
          method: "PATCH",
          headers: {
  ...
  const drawerList = () => (
    ...
    >
      <List>
        {["Service 1", "Service 2", "Service 3", "Service 4"].map((text, index) => (
      ...
      >
        <div className="alc-main-card-header">
          ...
          <div className="alc-main-card-header-services flex justify-between">
            ...
            <span>
              ...
              <span className="alc-card-lfont">{appointmentData?.date}</span>
            </span>
          </div>
        </div>
        ...
        <div className="alc-main-card-staff-details">
          <div>
            ...
            <span className="alc-card-lfont">{appointmentData?.staff_name}</span>
          </div>
        </div>
        <div className="alc-main-card-customer-details">
          ...
          <div>
            ...
            <span className="alc-card-lfont">₹ {appointmentData?.amount_paid}</span>
          </div>
        </div>
        <div className="alc-main-card-footer-btn">
          ...
          <button className="alc-card-cancel-btn" onClick={() => setShowCancelModal(true)}>
            Cancel
          </button>
          <button className="alc-card-extend-btn" onClick={() => setShowExtendModal(true)}>
            Extend
          </button>
          {isRunning ? (
            ...
            <button className="alc-card-start-timer-btn" onClick={handleStartTimer}>
              Start Timer
            </button>
          )}
