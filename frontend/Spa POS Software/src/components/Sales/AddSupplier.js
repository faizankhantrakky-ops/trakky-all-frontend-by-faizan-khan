...
const AddSupplier = ({ supplier }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!name || !supplier_description || !first_name || !last_name || !mobile_no || !email) {
    ...
    const url = isEdit
      ? `https://backendapi.trakky.in/spavendor/supplier/${formData.id}/` // Edit existing supplier
      : 'https://backendapi.trakky.in/spavendor/supplier/'; // Create new supplier
    ...
    try {
      ...
      if (response.ok) {
        const successMessage = isEdit ? 'Supplier edited successfully!' : 'Supplier submitted successfully!';
        ...
        toast.error(`Error saving supplier: ${errorData.detail || 'Unknown error'}`);
      }
    } catch (error) {
  ...
  return (
    ...
    <div className="flex justify-around px-5 mt-4 gap-8 max-sm:px-0 max-sm:flex-col max-sm:items-center">
      ...
      <div className="w-full max-w-lg space-y-6 px-5 py-5 bg-white rounded-lg shadow-md">
        ...
        <button className="bg-red-500 text-white px-6 py-2 rounded-lg" onClick={handleSubmit}>
          Save
        </button>
      </div>
      </div>
