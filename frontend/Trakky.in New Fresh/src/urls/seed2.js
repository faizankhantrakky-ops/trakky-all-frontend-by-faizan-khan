...
// Kids Special Salons: https://trakky.in/:city/childrenspecialsalon
...
// Kids Special Salons by Area: https://trakky.in/:city/childrenspecialsalon/:area
...
const ListSalonServicesLink = () => {
  ...
  cities?.map((city) => {
    links.push(`https://trakky.in/${city}/childrenspecialsalon/`);
  });
...
const ListSalonServicesLinkByArea = () => {
  ...
  areas?.map((area) => {
    let city = Object.keys(area)[0];
    area[city]?.map((area) => {
      links.push(`https://trakky.in/${city}/childrenspecialsalon/${area}`);
    });
  });
