...
const AreaListPage = ({ title, subtitle, name }) => {
  ...
  const handleFemaleBeautySalons = async () => {
    setIsDataLoading(true);
    let data = await getFemaleBeautySalons(city, area, page);
    setListData((prevValue) => [...prevValue, ...data.results]);
    if (data?.next) {
      setHasMore(true);
    } else {
  ...
  useEffect(() => {
    ...
    name == "childrenspecialsalon" && handleKidsSalons();
  ...
  const getMetaDescDetails = () => {
    ...
    } else if (name === "childrenspecialsalon") {
    ...
    <>
      <Helmet>
        ...
        {name === "childrenspecialsalon" && (
          <link
            rel="canonical"
            href={`https://trakky.in/${params?.city}/childrenspecialsalon/${params?.area}`}
          />
        )}
      </Helmet>
